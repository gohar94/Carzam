...
class yolo_tf:
    ...
    result_list = None
    # TODO See if we need all the classes here; might just need to keep car and bus
...
def draw_results(img, yolo):
    ...
    for i in range(len(results)):
        ...
        w = int(results[i][3])//2
        h = int(results[i][4])//2
        # this takes (x1, y1), (x2, y2)
        cv2.rectangle(img_cp,(x-w,y-h),(x+w,y+h),(0,0,255),4)
        # cv2.rectangle(img_cp,(x-w,y-h-20),(x+w,y-h),(125,125,125),-1)
        img_cp2 = img[y-h:y+h, x-w:x+w].copy()
        name = "new" + str(i) + ".jpg"
        cv2.imwrite(name, img_cp2)
        print("original shape was " + str(img.shape))
        print("new shape is " + str(img_cp2.shape))
...
def vehicle_detection_yolo(image):
    ...
    yolo_result = draw_results(image, yolo)
    return yolo_result
